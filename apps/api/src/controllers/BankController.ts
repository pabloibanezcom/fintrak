...
export const createRequisition = async (req: Request, res: Response): Promise<void> => {
  try {
    const { institutionId, redirect, reference } = req.body as CreateRequisitionRequest;
    ...
    const requisition = await GoCardlessService.createRequisition(requisitionData);
    ...
    const accountsWithDetails = await Promise.allSettled(
      accounts.map(async (accountId) => {
        try {
          ...
          console.error(`Failed to get details for account ${accountId}:`, error);
          return null;
        }
      })
    );
...
export const getTransactions = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (sync === 'true') {
      await syncAccountTransactions(accountId, userId, dateFrom as string, dateTo as string);
    }
    ...
    const transactions = await BankTransaction
      .find(filter)
      .sort({ bookingDate: -1 })
      .limit(1000);
...
export const syncTransactions = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await syncAccountTransactions(accountId, userId, dateFrom, dateTo);
...
async function syncAccountTransactions(
  ...
  try {
    const result = await GoCardlessService.getAccountTransactions(accountId, dateFrom, dateTo);
    const allTransactions = [...result.transactions.booked, ...result.transactions.pending];

    for (const transaction of allTransactions) {
      try {
        const transactionData: Partial<IBankTransaction> = {
          transactionId: transaction.transactionId || transaction.internalTransactionId || `${accountId}_${Date.now()}`,
          ...
          additionalInformationStructured: transaction.additionalInformationStructured,
