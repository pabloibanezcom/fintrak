    ...
    return (
      <div className={styles.page}>
        <Card padding="lg" className={styles.error}>
          <span className={styles.errorText}>{error || 'Transaction not found'}</span>
      ...
      <div className={styles.content}>
        <Card padding="lg" className={styles.mainCard}>
          <div className={styles.amountSection}>
            ...
            <span className={`${styles.typeBadge} ${isExpense ? styles.expense : styles.income}`}>
              {transaction.type}
            </span>
          </div>
          ...
          <div className={styles.detailsGrid}>
            <div className={styles.detailItem}>
              ...
              <span className={styles.detailValue}>{transaction.category.name}</span>
            </div>
            <div className={styles.detailItem}>
            ...
            {transaction.counterparty && (
              <div className={styles.detailItem}>
                ...
                <span className={styles.detailValue}>{transaction.counterparty.name}</span>
              </div>
            )}
            <div className={styles.detailItem}>
              <span className={styles.detailLabel}>Periodicity</span>
              <span className={styles.detailValue}>
                {transaction.periodicity === 'NOT_RECURRING' ? 'One-time' : transaction.periodicity}
              </span>
            </div>
          </div>
        ...
        <Card padding="md" className={styles.sideCard}>
          ...
          <div className={styles.metaItem}>
            ...
            <span className={styles.metaValue}>{formatDate(transaction.createdAt)}</span>
          </div>

          <div className={styles.metaItem}>
            <span className={styles.metaLabel}>Last Updated</span>
            <span className={styles.metaValue}>{formatDate(transaction.updatedAt)}</span>
          </div>
          ...
          {transaction.bankTransactionId && (
            ...
            >
              <span className={styles.linkedLabel}>Linked to bank transaction</span>
